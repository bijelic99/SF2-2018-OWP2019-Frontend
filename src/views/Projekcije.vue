          ...
          <v-card>
            <v-form v-model="searchData.formValid">
              ...
              <v-list-item>
                ...
                <v-text-field class="mr-2" label="Od" ref="cenaOdTF"/><v-text-field label="Do" ref="cenaDoTF"/>
              </v-list-item>
              <v-divider/>
              <v-list-item>
  ...
  export default {
    ...
    methods: {
      ...
      customFilterFunction: function(value, search, item) {
        ...
        if(!isGood) return isGood
        if(this.searchData.cenaOd !== null) isGood = isGood && item.cenaKarte >= this.searchData.cenaOd
        if(!isGood) return isGood
        if(this.searchData.cenaDo !== null) isGood = isGood && item.cenaKarte <= this.searchData.cenaDo
        if(!isGood) return isGood
